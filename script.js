/* ---------- SPA / NAVEGAÇÃO ---------- */
// Seleciona as seções principais do site
const sections = Array.from(document.querySelectorAll('section.hero, section#produtos, section#sobre, section#contato'));
let currentIndex = 0;
...
function nextPage() {
    showPage(Math.min(currentIndex + 1, sections.length - 1));
}

function prevPage() {
    showPage(Math.max(currentIndex - 1, 0));
}
    ...
    document.querySelectorAll('.yt-player').forEach((div) => {
        ...
        const player = new YT.Player(div.id, {
            ...
            events: {
                ...
                onStateChange: (ev) => {
                    // Avança automaticamente ao terminar (para TODOS os vídeos)
